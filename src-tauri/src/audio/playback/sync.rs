// PLL Constants - Optimized for stability and no oscillations
pub(crate) const PLL_KP: f32 = 0.0003; // Much more conservative proportional gain
pub(crate) const MAX_PLL_PITCH_ADJUSTMENT: f32 = 0.005; // Smaller max adjustment to prevent overshoot
pub(crate) const PLL_KI: f32 = 0.0001; // Reduced integral gain to prevent windup
pub(crate) const MAX_PLL_INTEGRAL_ERROR: f32 = 0.5; // Smaller integral error limit
pub(crate) const PLL_DEAD_ZONE: f32 = 0.01; // Dead zone to prevent micro-corrections
    ...
    let master_info = match local_states.get(master_deck_id_str) {
        Some(master_state) => {
            if master_state.duration <= Duration::ZERO {
                log::warn!(
                    "Audio Thread: EnableSync: Master deck '{}' is not loaded.",
                    master_deck_id_str
                );
                emit_error_event(
                    app_handle,
                    slave_deck_id_str,
                    &format!(
                        "Master deck '{}' must be loaded to sync",
                        master_deck_id_str
                    ),
                );
                return Ok(());
            ...
            if master_state.original_bpm.is_none() {
                log::warn!(
                    "Audio Thread: EnableSync: Master deck '{}' missing BPM.",
                    master_deck_id_str
                );
                emit_error_event(
                    app_handle,
                    slave_deck_id_str,
                    &format!("Master deck '{}' missing BPM", master_deck_id_str),
                );
                return Ok(());
            }
            Some((
        ...
        None => {
            ...
            emit_error_event(
                app_handle,
                slave_deck_id_str,
                &format!("Master deck '{}' not found", master_deck_id_str),
            );
            return Ok(());
        }
    };
    if let Some((master_bpm, master_current_pitch)) = master_info {
        let calculated_target_rate_for_slave = {
            if let Some(slave_state) = local_states.get_mut(slave_deck_id_str) {
                if slave_state.original_bpm.is_none() {
                    log::warn!(
                        "Audio Thread: EnableSync: Slave deck '{}' missing BPM.",
                        slave_deck_id_str
                    );
        ...
        if slave_deck_id_str != master_deck_id_str {
            if let Some(master_state_mut) = local_states.get_mut(master_deck_id_str) {
                ...
                log::error!(
                    "EnableSync: Failed to get mutable master state '{}' after initial check.",
                    master_deck_id_str
                );
            }
        }
        ...
        log::info!(
            "EnableSync: Performing direct phase alignment for slave '{}' to master '{}'",
            slave_deck_id_str,
            master_deck_id_str
        );

        // Get current positions directly - no complex timing needed
        if let (Some(master_state), Some(slave_state)) = (
            local_states.get(master_deck_id_str),
            local_states.get(slave_deck_id_str),
        ) {
...
pub(crate) fn audio_thread_handle_disable_sync<R: Runtime>(
    ...
    let (pitch_to_restore_this_deck, was_master_before_disable, id_of_former_master_if_slave) = {
        let deck_state = local_states.get_mut(deck_id_str).ok_or_else(|| {
            crate::audio::errors::PlaybackError::DeckNotFound {
                deck_id: deck_id_str.to_string(),
            }
        })?;

        if !deck_state.is_sync_active && !deck_state.is_master {
            log::warn!(
                "DisableSync: Deck '{}' is not currently synced or master. No action needed.",
                deck_id_str
            );
            return Ok(());
        }
    ...
    log::info!(
        "Deck '{}' sync/master status disabled. Will restore its pitch to: {:.4}",
        deck_id_str,
        pitch_to_restore_this_deck
    );
    ...
    if was_master_before_disable {
        log::info!(
            "Deck '{}' was master. Finding and disabling sync for its former slaves...",
            deck_id_str
        );
        let slaves_of_this_deck: Vec<String> = local_states
            .iter()
            .filter(|(id, state)| {
                *id != deck_id_str && state.master_deck_id.as_deref() == Some(deck_id_str)
            })
            .map(|(id, _)| id.clone())
            .collect();
        ...
        if !slaves_of_this_deck.is_empty() {
            log::info!(
                "Disabling sync for former slaves of '{}': {:?}",
                deck_id_str,
                slaves_of_this_deck
            );
            for slave_id_str_of_disabled_master in slaves_of_this_deck {
                ...
                audio_thread_handle_disable_sync(
                    &slave_id_str_of_disabled_master,
                    local_states,
                    app_handle,
                )?;
            }
    ...
    for master_to_check_id in potential_masters_to_demote {
        ...
        if !master_still_has_active_slaves {
            ...
            let should_demote_this_master = local_states
                .get(&master_to_check_id)
                .map_or(false, |s| s.is_master);

            if should_demote_this_master {
                log::info!(
                    "Audio Thread: Former master '{}' (of a now-disabled slave) no longer has other active slaves. Disabling its master status.",
                    master_to_check_id
                );
...
pub(crate) fn calculate_pll_pitch_updates(
    ...
    for (deck_id, deck_state) in local_states {
        ...
        let Some(master_id) = &deck_state.master_deck_id else {
            continue;
        };
        let Some(slave_bpm) = deck_state.original_bpm else {
            continue;
        };
        let Some(slave_fbs) = deck_state.first_beat_sec else {
            continue;
        };
        let Some(slave_time) = deck_times.get(deck_id).map(|(t, _)| *t) else {
            continue;
        };

        let Some(master_state) = local_states.get(master_id) else {
            continue;
        };
        let Some(master_bpm) = master_state.original_bpm else {
            continue;
        };
        let Some(master_fbs) = master_state.first_beat_sec else {
            continue;
        };
        let Some(master_time) = deck_times.get(master_id).map(|(t, _)| *t) else {
            continue;
        };
        ...
        let master_phase =
            ((master_time - master_fbs as f64).max(0.0) / master_beat_interval) % 1.0;
        ...
        // Apply dead zone to prevent micro-corrections that cause oscillations
        let error_f32 = error as f32;
        if error_f32.abs() < PLL_DEAD_ZONE {
            continue; // Skip tiny corrections that cause instability
        }
